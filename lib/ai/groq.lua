...
local function createLanguageModel(modelId, config)
  ...
  function model.doGenerate(opts)
    ...
    if not apiKey or apiKey == "" then
      error("API key is required. Set GROQ_API_KEY environment variable or pass apiKey to createGroq().")
    end
...
local function createGroq(options)
  options = options or {}
  local baseURL = options.baseURL or DEFAULT_BASE_URL
  local providerName = options.name or "groq"

  local function getApiKey()
    return options.apiKey or os.getenv("GROQ_API_KEY")
  end
